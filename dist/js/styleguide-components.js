...
!function(angular) {
    ...
    function TwDateLookupController($element, $scope, $timeout, TwDateService) {
        ...
        function addWatchers() {
            ...
            }), $scope.$on(COMPONENT_EVENTS.CALENDAR_MODEL_UPDATED, function() {
        ...
        function updateCalendarView(viewDate) {
            ...
            $scope.$broadcast(COMPONENT_EVENTS.CALENDAR_MODEL_UPDATED);
        }
        ...
        $ctrl.openLookup = function() {
            ...
            setModel(TwDateService.getUTCDateFromParts(year, month, day)), resetFocus(), void $scope.$broadcast(COMPONENT_EVENTS.CALENDAR_MODEL_UPDATED));
        }, $ctrl.selectMonth = function($event, month, year) {
            $event.stopPropagation(), $ctrl.isMonthDisabled(month, year) || ($ctrl.month = month, 
            $ctrl.weeks = getTableStructure(), $ctrl.mode = "day", $scope.$broadcast(COMPONENT_EVENTS.CALENDAR_MODEL_UPDATED));
            ...
            $ctrl.weeks = getTableStructure(), $scope.$broadcast(COMPONENT_EVENTS.CALENDAR_MODEL_UPDATED);
        }, $ctrl.yearBefore = function($event) {
            $event.stopPropagation(), $ctrl.year--, $ctrl.weeks = getTableStructure(), $scope.$broadcast(COMPONENT_EVENTS.CALENDAR_MODEL_UPDATED);
    ...
    function TwDateService() {
        ...
        this.getLocaleDate = function(date) {
            ...
            var date, days = [], predefinedDayNames = getLocaleSpecificDayNames(locale);
            if (predefinedDayNames) return predefinedDayNames;
            ...
            var date, months = [], predefinedMonthNames = getLocaleSpecificMonthNames(locale);
            if (predefinedMonthNames) return predefinedMonthNames;
            format = getValidDateFormat(format), locale = getValidLocale(locale);
