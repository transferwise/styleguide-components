!function(angular) {
    ...
    function TwFormGroup() {
        return {
            restrict: "C",
            link: function(scope, formGroup) {
                var potentialParents = ".form-group, .checkbox > label, .radio > label", formControls = $(formGroup).find(".form-control");
    ...
    angular.module("tw.styleguide-components").directive("formGroup", TwFormGroup);
}(window.angular), function(angular) {
    "use strict";
    function TwInvalid() {
        return {
    ...
    function TwInput() {
        ...
        var checkboxTemplate = "<button type='button' class='input-replacement'><span class='glyphicon glyphicon-ok'></span></button>", radioTemplate = "<button type='button' class='input-replacement'><span></span></button>", disabledReplacement = "<span class='disabled-replacement input-replacement'><span><span></span>";
        return {
            restrict: "E",
            link: function(scope, element, attrs) {
                if (attrs.type) {
                    ...
                    if ("radio" === type || "checkbox" === type) {
                        if ("radio" === type) {
                            var radioReplace = $(radioTemplate).keypress(onKeypress).click(onClick).focus(onFocus).blur(onBlur);
                            $(element).hide().after(radioReplace), radioReplace.after(disabledReplacement);
                        }
                        if ("checkbox" === type) {
                            var checkReplace = $(checkboxTemplate).keypress(onKeypress).click(onClick).focus(onFocus).blur(onBlur);
                            $(element).hide().after(checkReplace), checkReplace.after(disabledReplacement);
                        }
                    }
                }
            }
