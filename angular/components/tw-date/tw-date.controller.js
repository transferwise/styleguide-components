	...
	function TwDateController($scope, $element) {
		...
		function getMonths(locale) {
			...
			if (!date.toLocaleString) {
				// Browser doesn't support toLocaleString, use English
			...
			try {
				var testToLocaleString = date.toLocaleString(locale, {month: "long"});

				if (safariRegex.test(testToLocaleString)) {
					return getEnglishMonths();
				}
			} catch(ex) {
				return getEnglishMonths();
		...
		function updateDate() {
			...
			console.log(vm.ngMin);
			console.log(vm.ngMax);
			console.log(minObj);
			console.log(maxObj);
			console.log(dateObj);
